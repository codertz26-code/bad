...
// ============ OWNER CONFIGURATION - FIXED ============
const ownerNumber = ['255768978061'] // Tafadhali hakikisha namba yako iko hapa
...
> © 𝐒𝐈𝐋𝐀 𝐌𝐃 | Crafted with precision`;
  ...
  conn.ev.on('messages.upsert', async(mek) => {
    ...
    // ============ FIXED ADMIN CHECK ============
    // Check if sender is admin (convert both to same format)
    ...
    if (isGroup && senderNumber === '255768978061') {
      console.log(`🔍 DEBUG - Sender: ${senderNumber}, isAdmins: ${isAdmins}, isOwner: ${isOwner}, groupAdmins: ${JSON.stringify(groupAdmins.map(a => a.split('@')[0]))}`);
    }
    ...
    if (config.MODE === "private") {
      ...
      // Inbox mode: Only private chats (not groups)
      shouldProcess = !isGroup || isOwner;
    } else if (config.MODE === "groups") {
      // Groups mode: Only groups (not private chats)
      shouldProcess = isGroup || isOwner;
    }
    
    // If shouldProcess is false, skip command processing
